...
const Login = () => {
  ...
  // const formik = useFormik({
  //   initialValues: {
  //     email: 'maira@gmail.com', // Hardcoded default email
  //     password: 'maira123@', // Hardcoded default password
  //   },
  //   validationSchema: LoginValidationSchema,
  //   onSubmit: (values) => {
  //     loginUser(values); // Call updateStaff function to handle form submission
...
const formik = useFormik({
  initialValues: {
    email: '', // Set to empty to allow user input
    password: '', // Set to empty to allow user input
  },
  validationSchema: LoginValidationSchema,
  onSubmit: (values) => {
    loginUser(values); // Handle form submission
  },
});
...
const loginUser = async (credentials) => {
  try {
    ...
    console.log('Login successful, data:', data); // Debugging line
    dispatch(setCredentials(data));
    navigate('/dashboard');
  } catch (error) {
    const errorMessage = error?.error?.message || error?.data?.error?.message || 'An error occurred.';
    console.error('Login failed, error:', error); // Debugging line
